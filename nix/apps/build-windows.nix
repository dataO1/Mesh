...
pkgs.writeShellApplication {
  ...
  # SC2016: Variables inside 'bash -c' single-quoted block are intentionally
  # expanded by the container's shell, not the outer shell.
...
TOOLCHAIN
        ...
        if [[ -n "$CLANG_INCLUDE" ]]; then
          export BINDGEN_EXTRA_CLANG_ARGS="-I$CLANG_INCLUDE -D_GCC_MAX_ALIGN_T"
        fi
...
PKGWRAPPER
        ...
        CLANG_INCLUDE=$(find /usr/lib/llvm-*/lib/clang -path "*/include/stdbool.h" -printf "%h\n" 2>/dev/null | head -1)
        export BINDGEN_EXTRA_CLANG_ARGS="--sysroot=/usr/x86_64-w64-mingw32 -I/usr/x86_64-w64-mingw32/include -D_GCC_MAX_ALIGN_T ''${GCC_INCLUDE:+-I$GCC_INCLUDE} ''${CLANG_INCLUDE:+-I$CLANG_INCLUDE}"
