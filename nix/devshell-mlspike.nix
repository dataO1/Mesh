...
in pkgs.mkShell {
  ...
  shellHook = ''
    ...
    box ""
    box "  3. Text-LLM rating — N independent jurors. Each tag writes its own"
    box "     NPZ; step 5 (v18-train) auto-aggregates them via Dawid-Skene."
    box ""
    box "  ★ START 3A + 3B AS SOON AS STEP 1 IS UP — they run on REMOTE GPUs"
    box "    (Spark 1, Spark 2), so they share nothing with caption-full and"
    box "    overlap fully with step 2. The streaming runner polls the"
    box "    captions dir every 5 min and exits ~20 min after step 2 ends."
    box ""
    box "     3A) Spark 2 / Nemotron-30B (terminal C, parallel with step 2)"
    ...
    box "       export TEXT_LLM_TAG=nemotron  TEXT_LLM_NO_THINK=1"
    box "       ./...pipeline.sh caption-rate-streaming"
    box ""
    box "     3B) Spark 1 / Qwen3.6-27B (terminal D, parallel with step 2)"
    box "       export TEXT_LLM_URL=http://172.16.51.90:8000/v1/chat/completions"
    box "       export TEXT_LLM_MODEL=sakamakismile/Qwen3.6-27B-Text-NVFP4-MTP"
    box "       export TEXT_LLM_TAG=qwen36  TEXT_LLM_NO_THINK=1  TEXT_LLM_WORKERS=16"
    box "       ./...pipeline.sh caption-rate-streaming"
    box ""
    box "     3C) Local Mistral-Small-3.2-24B AWQ — SEQUENTIAL, after step 2:"
    box "       pkill -f 'python.*api_server'         # frees GPU from MF"
    box "       TEXT_LLM_MODEL=gghfez/Mistral-Small-3.2-24B-Instruct-hf-AWQ \\"
    box "         bash spike/track-grading/serve_text_llm.sh   # :8002"
    box "       TEXT_LLM_TAG=local_mistral \\"
    box "         bash ...pipeline.sh caption-rate"
    box "     (Devstral-Small-2507 ships bf16-only ~48 GB → won't fit 24 GB"
    box "      5090. Mistral-Small-3.2 is Devstral's pre-code-fine-tune base."
    box "      For natural-language intensity rating the base is fine —"
    box "      arguably better-calibrated than the code-tuned variant.)"
    box ""
    box "     Tail-latency tweak: STABLE_SECS=300 POLL_SECS=60 ./...streaming"
    box ""
