...
in pkgs.mkShell {
  ...
  shellHook = ''
    ...
    box ""
    box "  3. Text-LLM rating — parallel (remote) OR sequential (local)"
    box "     parallel:  terminal C, runs alongside step 2"
    box "       export TEXT_LLM_URL=http://<host>:8000/v1/chat/completions"
    box "       export TEXT_LLM_MODEL=<served model id>"
    box "       export TEXT_LLM_TAG=nemotron  TEXT_LLM_NO_THINK=0"
    box "       ./...pipeline.sh caption-rate-streaming"
    box "     sequential: after step 2 finishes"
    box "       pkill -f 'python.*api_server'      # free GPU"
    box "       TEXT_LLM_MODEL=Qwen/Qwen2.5-7B-Instruct \\"
    box "         bash spike/track-grading/serve_text_llm.sh       # :8002"
    box "       bash spike/track-grading/run_round7_6_pipeline.sh caption-rate"
    box ""
    ...
    box "  Stop:    pkill -f 'python.*api_server'   Resume: re-run any step"
