...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    echo "╠═══════════════════════════════════════════════════════════════════════╣"
    echo "║  ONNX Model Conversion:                                                ║"
    echo "║    nix run .#convert-model        # Demucs PyTorch → ONNX             ║"
    echo "║    nix run .#convert-ml-model     # Essentia TF classification → ONNX ║"
    echo "╠═══════════════════════════════════════════════════════════════════════╣"
    echo "║  PD Effects (Pure Data neural audio effects):                         ║"
    echo "║    nix run .#build-nn-tilde       # Build nn~ external for RAVE       ║"
    echo "║                                                                       ║"
    echo "║  nn~ Installation:                                                    ║"
    echo "║    1. nix run .#build-nn-tilde    # Builds to dist/nn~/               ║"
    echo "║    2. cp dist/nn~/nn~.pd_linux .../effects/externals/                 ║"
    echo "║                                                                       ║"
    ...
    echo "║    nix run .#bpm-report -- --no-scrape # Report from cached GT only ║"
    echo "║                                                                       ║"
    echo "║  Outputs: bpm-comparison.json, ground-truth.json (edit to fix)       ║"
    echo "╠═══════════════════════════════════════════════════════════════════════╣"
    echo "║  Embedded (Orange Pi 5 Pro, aarch64):                                ║"
    echo "║    nix run .#embedded-setup          # one-time CI keypair + Pages ║"
