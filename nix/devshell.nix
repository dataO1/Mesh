...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    echo "║    nix build .#sdImage                # Build NixOS SD card image    ║"
    echo "║    nixos-rebuild switch --fast \\                                      ║"
    echo "║      --flake .#mesh-embedded \\                                       ║"
    echo "║      --target-host mesh@<ip> --use-remote-sudo  # Deploy update     ║"
    echo "║                                                                       ║"
